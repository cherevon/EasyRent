...
void DlgUserQueryEditor::loadQueryDataFromUI() const
{
    if (!fRefreshingInfo)
    {
        // тип запроса
...
void DlgUserQueryEditor::updateLastFloorLimits() const
{
...
RealEstateQuery *DlgUserQueryEditor::editQuery() const
{
    return fEditQuery;
}


void DlgUserQueryEditor::setEditQuery(RealEstateQuery *value)
{
    fInitialQuery = value;
...
void DlgUserQueryEditor::refreshInfo()
{
...
void DlgUserQueryEditor::cancel()
{
    fEditQuery->assign(fInitialQuery);
    fEditQuery->searchObject()->assign(fInitialQuery->searchObject());
